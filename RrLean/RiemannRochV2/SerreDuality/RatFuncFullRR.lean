...
For effective divisors D with linear place support on P¹ (g = 0):
  ℓ(D) - ℓ(K - D) = deg(D) + 1 - g = deg(D) + 1

Note: The abstract `FullRRData` class uses `ell_proj` which is based on the affine L(D).
For RatFunc Fq, we need the projective L(D) with the "no pole at infinity" constraint.
Therefore, we state the theorem directly using `ell_ratfunc_projective`.
-/

/-- The full Riemann-Roch theorem for P¹ over Fq.

For any effective divisor D with linear place support:
  ℓ(D) - ℓ(K - D) = deg(D) + 1

where K is the canonical divisor and g = 0 for P¹.

This is the complete Riemann-Roch formula for the projective line. -/
theorem riemann_roch_ratfunc (D : DivisorV2 (Polynomial Fq))
    ...
    D.deg + 1 - (genus_ratfunc : ℕ) := by
  -- For genus 0: the formula simplifies to ℓ(D) - ℓ(K-D) = deg(D) + 1
  -- We'll show both terms separately

  -- ℓ(D) = deg(D) + 1 for effective D with linear support
  have h1 : ell_ratfunc_projective D = D.deg.toNat + 1 :=
    ell_ratfunc_projective_eq_deg_plus_one Fq D hD hDlin

  -- For effective D, deg(D) ≥ 0
  have hdeg_nn : 0 ≤ D.deg := DivisorV2.deg_nonneg_of_effective hD

  -- Since deg(D) ≥ 0 ≥ -1, ℓ(K-D) = 0
  have h2 : ell_ratfunc_projective (canonical_ratfunc Fq - D) = 0 :=
    ell_canonical_sub_zero Fq D (by omega) hDlin

  -- Combine
  simp only [h1, h2, Int.ofNat_add, genus_ratfunc, Nat.cast_zero, sub_zero]
  rw [Int.toNat_of_nonneg hdeg_nn]
  ring

/-- Riemann-Roch for D = 0: ℓ(0) - ℓ(K) = 1 - 0 = 1.
Since K has degree -2 < 0, ℓ(K) = 0, confirming ℓ(0) = 1. -/
theorem riemann_roch_at_zero :
    (ell_ratfunc_projective (0 : DivisorV2 (Polynomial Fq)) : ℤ) -
    ell_ratfunc_projective (canonical_ratfunc Fq - 0) = 1 := by
  have h0_eff : (0 : DivisorV2 (Polynomial Fq)).Effective := fun _ => le_refl 0
  have h0_lin : IsLinearPlaceSupport (0 : DivisorV2 (Polynomial Fq)) := fun _ h => by
    simp only [Finsupp.mem_support_iff, Finsupp.coe_zero, Pi.zero_apply, ne_eq,
               not_true_eq_false] at h
  have h := riemann_roch_ratfunc Fq 0 h0_eff h0_lin
  simp only [DivisorV2.deg_zero, genus_ratfunc, Nat.cast_zero, sub_zero, zero_add] at h
  exact h

/-- Riemann-Roch lower bound: ℓ(D) ≥ deg(D) + 1 for effective D with linear support.
This is equivalent to the Riemann inequality for g = 0. -/
theorem riemann_inequality_ratfunc (D : DivisorV2 (Polynomial Fq))
    (hD : D.Effective) (hDlin : IsLinearPlaceSupport D) :
    (ell_ratfunc_projective D : ℤ) ≥ D.deg + 1 := by
  have h1 : ell_ratfunc_projective D = D.deg.toNat + 1 :=
    ell_ratfunc_projective_eq_deg_plus_one Fq D hD hDlin
  have hdeg_nn : 0 ≤ D.deg := DivisorV2.deg_nonneg_of_effective hD
  simp only [h1, Int.ofNat_add]
  rw [Int.toNat_of_nonneg hdeg_nn]
  linarith

/-- Exact dimension formula: ℓ(D) = deg(D) + 1 for effective D with linear support.
This is the full dimension formula for P¹. -/
theorem ell_eq_deg_plus_one_ratfunc (D : DivisorV2 (Polynomial Fq))
    (hD : D.Effective) (hDlin : IsLinearPlaceSupport D) :
    (ell_ratfunc_projective D : ℤ) = D.deg + 1 := by
  have h1 : ell_ratfunc_projective D = D.deg.toNat + 1 :=
    ell_ratfunc_projective_eq_deg_plus_one Fq D hD hDlin
